...
int process_fetch(core_t *core, warrior_t *warrior, process_t **process)
{
    u_int8_t it;
    (*process)->state = WAIT;
    sfText_setOutlineColor(core->reveal.cell[(*process)->it.pc].text, sfBlack);
    (*process)->it.pc = (*process)->pc;
    sfText_setOutlineColor(core->reveal.cell[(*process)->it.pc].text, sfWhite);
    ...
    (*process)->cycle = ref[it].nbr_cycles;INC_PC((*process)->pc);
    if (fetch_particular(core, warrior, process))
        return (0);
    fetch_type(core, warrior, process);
    INC_PC((*process)->pc);
    fetch_param(core, warrior, process);
    fill_cmd(core, warrior, process, &core->reveal.info[warrior->n - 1].cmd);
    return (0);
}